...
if (isset($_GET["type"]) && $_GET["type"] == "monstros") {
...
} elseif (isset($_GET["type"]) && ($_GET["type"] == "equipamentos" || $_GET["type"] == "consumiveis" || $_GET["type"] == "diversos")) {
    // Handle new item categories
    $type = $_GET["type"];

    // Define title and type filter based on category
    if ($type == "equipamentos") {
        $title = 'Equipamentos';
        $typeFilter = "('weapon', 'armor', 'shadowgear')";
    } elseif ($type == "consumiveis") {
        $title = 'Consumíveis';
        $typeFilter = "('healing', 'usable', 'delayconsume')";
    } else {
        $title = 'Itens Diversos';
        $typeFilter = "('etc', 'card', 'petegg', 'petarmor', 'ammo')";
    }

    if (empty($pagina) || !isset($pagina)) {
        header("Location: ?to=database&type=" . $type . "&page=1");
        exit();
    }
    ...
    if (isset($_GET["busca"]) && !empty($_GET["busca"])) {
        ...
        try {
            $sql = "SELECT * FROM item_db WHERE (name_english LIKE ? OR id LIKE ?) AND type IN " . $typeFilter . " ";
            $sql .= "UNION ";
            $sql .= "SELECT * FROM item_db2 WHERE (name_english LIKE ? OR id LIKE ?) AND type IN " . $typeFilter . " ";
            $sql .= "ORDER BY id";
        ...
        try {
            $sql = "SELECT * FROM item_db WHERE type LIKE ? AND type IN " . $typeFilter . " ";
            $sql .= "UNION ";
            $sql .= "SELECT * FROM item_db2 WHERE type LIKE ? AND type IN " . $typeFilter . " ";
            $sql .= "ORDER BY id";
        ...
        try {
            $sql = "SELECT * FROM item_db WHERE type IN " . $typeFilter . " ";
            $sql .= "UNION ";
            $sql .= "SELECT * FROM item_db2 WHERE type IN " . $typeFilter . " ";
            $sql .= "ORDER BY id";
...
} elseif (isset($_GET["type"]) && $_GET["type"] == "inicio") {
    // Database home page - no data needed
    $title = 'Database';
} else {
    // If no valid type, redirect to database inicio
